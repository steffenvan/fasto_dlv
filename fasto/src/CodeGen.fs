...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  | Replicate (n, a, _, (line, _)) ->
      let size_reg = newName "size_reg"
      let n_code = compileExp n vtable size_reg
      let val_reg = newName "val_reg"
      let a_code = compileExp a vtable val_reg
      (*  size_reg is now the integer n. 
          val_reg now contains type of a        
      *)
      ...
      *)
      ...
      (* replicate is just 'arr[i] = i'.  arr[i] is addr_reg. *)
      let loop_replicate   = [ Mips.SW (val_reg, addr_reg, "0") ]
